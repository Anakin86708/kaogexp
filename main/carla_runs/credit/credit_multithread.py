...
from main.carla_runs.util import save_tratador_and_normalizador, save_counterfactuals, compute_and_save_metrics
...
train_data = pd.read_csv(os.path.join(working_dir, '../../../data/carla_data/give_me_some_credit_train.csv'),
                         index_col=0)
train_data.columns = train_data.columns.str.strip()
train_data = DatasetFromMemory(train_data, index)

test_data = pd.read_csv(os.path.join(working_dir, '../../../data/carla_data/give_me_some_credit_test.csv'), index_col=0)
...
logging.info('Realizando explicacao...')
threads = []
